...
---
## 5. 결론
사용하면서 단점으로 뽑자면 Next.js 버전이 업그레이드 될때마다 혹은 새로운 feature가 나올때마다 `Vercel`에서 배포하는경우에는 별도의 설정이나 안되는경우 없이 바로 적용이 되는것 같지만 `SST` 아무래도 시간이 조금 걸리거나 별도의 설정을 해줘야되는 것이 단점인것같습니다.
<Br/>
개인적인 경험으로 `Streaming SSR`을 사용하려면 `open-next.config`에서 별도의 설정이 필요하고, `PPR(Partial Pre Rendering)`이 현재 `Next.js` 에서 experimental 버전에서만 사용 할 수 있다고했지만 `Vercel` 배포했을땐 적용이 잘되었지만 `SST`는 적용이 안되었습니다.
<Br/>
