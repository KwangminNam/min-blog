...
```bash
aws cloudfront associate-alias --alias <ROUTE53_DOMAIN> --target-distribution-id <CFID> --profile <PROFILE_NAME>
```
...
```bash
aws cloudfront get-distribution-config --id E1FLUF8QS5FENJ --profile kwangmin
```
...
- CloudFront 콘솔 확인:
- AWS Management Console에서 대상 배포(E1FLUF8QS5FENJ)를 열어 Alternate Domain Names 항목에 `www.example.com`이 표시되는지 확인합니다.

- DNS 확인:
터미널에서 `dig www.example.com` 명령어를 실행하여, 해당 도메인이 대상 배포의 CloudFront 도메인(예: d13cewiwipxf6x.cloudfront.net)을 가리키는지 확인할 수 있습니다.
<Hr/>

## 4. Trouble Shooting

**An error occurred (AccessDenied) when calling the ListConflictingAliases operation: User: arn:aws:iam::886436946542:user/kwangmin-aws-serverless is not authorized to perform: cloudfront:ListConflictingAliases on resource: arn:aws:cloudfront::886436946542:distribution/E157FEJO2A2BXR because no identity-based policy allows the cloudfront:ListConflictingAliases action**
