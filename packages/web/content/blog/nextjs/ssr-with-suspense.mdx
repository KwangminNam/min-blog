...
<Br/>
여러분이 Next.js를 이용해 프로젝트를 시작하거나 리팩토링할 계획이라면, 이번에 소개된 `App Router + RSC 개념과 Streaming Suspense` 도입을 진지하게 고민해 보는 것은 어떨까요? 과거 SSR보다 훨씬 개선된 `DX(Developer Experience)와 UX(User Experience)`를 얻을 수 있을 것 같습니다 :)
